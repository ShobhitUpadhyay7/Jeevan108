  ...
  try {
    ...
    if (profilePicture && typeof profilePicture === "string" && profilePicture.trim()) {
      ...
      if (profilePicture.startsWith("data:image/") || profilePicture.startsWith("data:application/")) {
        ...
        try {
          processedProfilePicture = await uploadToCloudinary(profilePicture, "image", "jeevan108/staff");
        } catch (picError) {
      ...
      if (profilePicture.startsWith("data:image/") || profilePicture.startsWith("data:application/")) {
        ...
        try {
          ...
          if (currentStaff.profilePicture) {
            try {
              await deleteFile(currentStaff.profilePicture as string, "image");
            } catch (delError) {
...
export async function deleteStaff(req: Request, res: Response) {
  try {
    ...
    if (staff.profilePicture) {
      try {
        await deleteFile(staff.profilePicture as string, "image");
      } catch (delError) {
